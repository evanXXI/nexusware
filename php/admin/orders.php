...
switch ($method["choice"]) {
    case "select":
        $req = $db->query("SELECT o.*, CONCAT(u.firstname, ' ', u.lastname) AS username, CONCAT(u.street_number, u.street_bis, ' ', u.street_name, ' ', u.zip_code, ', ', u.country) AS user_address FROM orders o INNER JOIN users u ON o.user_id = u.id;");

        if ($req) $orders = $req->fetchAll(PDO::FETCH_ASSOC);
        else $orders = [];
    ...
    case "insert":
        ...
        foreach (json_decode($method["products"]) as $product) {
            $req = $db->prepare("INSERT INTO ispartof(order_id, product_id, product_qty) VALUES (:order_id, :product_id)");
